...
# Here we store the normalised Fisher for an 8 qubit model with hardware noise
# If you want to scale up, change the dict that is hardcoded in line 165 and possibly elsewhere
# Please insert your own token in line 117
...
class EffectiveDimension:
    ...
    def get_fhat(self):
        ...
        """
        grads = np.zeros(shape=(self.num_thetas*self.num_inputs, self.d, 2**self.model.inputsize))
        output = np.zeros(shape=(self.num_thetas*self.num_inputs, 2))
        stepsize = int(self.num_inputs * self.num_thetas / 20)  # for batch gradients
        for i in range(stepsize, self.num_thetas*self.num_inputs+1, stepsize):
            params = self.params[i - stepsize: i]
            x = self.x[i - stepsize: i]
            batchgrad = self.model.get_gradient(params=params, x=x)  # get gradients, dp_theta
            grads[i - stepsize: i] = batchgrad
            output[i - stepsize: i] = self.model.forward(params=params, x=x)  # get model output
            print("percentage completed: ", (i*100)/(self.num_thetas*self.num_inputs))
