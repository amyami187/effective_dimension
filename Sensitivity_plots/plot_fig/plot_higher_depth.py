...
# n here to a sufficiently large value. In particular, this file looks at models with higher d (higher number of \
# parameters).
...
for ms in m:
    ed_m = []
    for seed in seeds:
        ed = np.load(path+"data/higher_depth/4in_depth9_effective_dimension_samples_%i_seed_%i.npy" %(ms, seed))
        ed_m += [ed]
    ed_m = np.array(ed_m)
...
for ms in m:
    ed_m = []
    for seed in seeds:
        ed = np.load(path+"data/higher_depth/6in_depth9_effective_dimension_samples_%i_seed_%i.npy" %(ms, seed))
        ed_m += [ed]
    ed_m = np.array(ed_m)
...
for ms in m:
    ed_m = []
    for seed in seeds:
        ed = np.load(path+"data/higher_depth/8in_depth9_effective_dimension_samples_%i_seed_%i.npy" %(ms, seed))
        ed_m += [ed]
    ed_m = np.array(ed_m)
...
for ms in m:
    ed_m = []
    for seed in seeds:
        ed = np.load(path+"data/higher_depth/10in_depth9_effective_dimension_samples_%i_seed_%i.npy" %(ms, seed))
        ed_m += [ed]
    ed_m = np.array(ed_m)
...
plt.plot(m, avg_ed4_n1, label="4, d = 40")
...
plt.plot(m, avg_ed6_n1, label="6, d = 60")
...
plt.plot(m, avg_ed8_n1, label="8, d = 80")
...
plt.plot(m, avg_ed10_n1, label="10, d = 100")
...
plt.plot(m, avg_ed4_n2, label="4, d = 40")
...
plt.plot(m, avg_ed10_n2, label="10, d = 100")
...
plt.plot(m, avg_ed4_n3, label="4, d = 40")
...
plt.plot(m, avg_ed10_n3, label="10, d = 100")

plt.legend(title="Input size")
plt.title("n = 1e13")
plt.savefig('sensitivity_higher_depth.pdf', format='pdf', dpi=1000)
plt.show()